...
interface FiltersProps {
    onFilterChange: (filters: { width: string | null, height: string | null, season: string | null, manufacturer: string | null, radius: string | null  }) => void;
}
...
const Filters = ({ onFilterChange }: FiltersProps) => {
    ...
    const [radius, setRadius] = useState<string | null>(null);
        ...
        console.log({ width, height, season, manufacturer,radius });         onFilterChange({ width, height, season, manufacturer, radius });
    };
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                ...
                <div>
                    <label htmlFor="radius" className="block text-sm font-medium text-gray-600">Радиус</label>
                    <select
                        id="radius"
                        name="radius"
                        className="mt-1 block w-full pl-4 pr-10 py-3 border border-gray-300 bg-white rounded-xl focus:outline-none focus:ring-blue-400 focus:border-blue-400 sm:text-sm shadow-sm"
                        value={radius || ''}
                        onChange={(e) => setRadius(e.target.value || null)}
                    >
                        <option value="">Все</option>
